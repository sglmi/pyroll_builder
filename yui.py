...
class Table(tk.Canvas):
    def __init__(self, parent, filename, *args, **kwargs):
        ...
        self.filename = filename
        self.bodyframe = ttk.Frame(self, relief="sunken", padding=50)
        self.bodyframe.pack(anchor="w")
        ...
        self.rowframe = ttk.Frame(self.bodyframe, relief="solid")
        self.rowframe.pack(anchor="w", fill=tk.X, pady=5)  # space betwen each row
    ...
    def header(self, *args, **kwargs):
        ...
        for index, title in enumerate(args, 1):
            ttk.Button(self.rowframe, text=title).grid(
                row=0, column=index, padx=(5, 40 + x)
            )
        x += 30
    ...
    def insert_row(self, eid, name, email):
        ...
        ttk.Label(self.rowframe, text=eid).grid(row=eid, column=1, sticky="w")
